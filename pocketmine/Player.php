   ...
   public function getPing(){return$this->ping;
	...
	public function setBanned($value){
		if($value === true){
			...
			$this->kick("§cEstas baneado puto Randy");
		}else{
	...
	protected function processLogin(){
		if(!$this->server->isWhitelisted(strtolower($this->getName()))){
			$configPath = \pocketmine\Server::getInstance()->getDataPath() . "genisys.yml";
			$mainConfig = (new \pocketmine\utils\Config($configPath, \pocketmine\utils\Config::YAML))->getAll();

			$whitelistMessage = "Server is white-listed";
			if(isset($mainConfig["wl-msg"])){
				$whitelistMessage = $mainConfig["wl-msg"];
			}elseif(isset($mainConfig["server"]["wl-msg"])){
				$whitelistMessage = $mainConfig["server"]["wl-msg"];
			}
			$whitelistMessage = str_replace("@player", $this->getName(), $whitelistMessage);

			$this->close($this->getLeaveMessage(), $whitelistMessage);
	 ...
	 * @param bool   $notify
	 */
	public final function close($message = "", $reason = "§cNi puta Idea crack", $notify = true){
