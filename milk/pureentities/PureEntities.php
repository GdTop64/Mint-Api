...
class PureEntities extends PluginBase implements Listener{
    ...
    $this->getServer()->getScheduler()->scheduleRepeatingTask(new AutoSpawnTask($this), 200);
        $classes = [
            Blaze::class,
            CaveSpider::class,
            Chicken::class,
    ...
    public function BlockBreakEvent(BlockBreakEvent $ev){
        ...
        if(
            ...
            ) && ($block->level->getBlockLightAt((int) $block->x, (int) $block->y, (int) $block->z) < 12 and mt_rand(1, 70) < 2)
        ){
